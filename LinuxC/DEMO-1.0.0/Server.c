...
void eventadd(int efd, int events, struct myevent_s *ev)
{
    ...
    if (epoll_ctl(efd, op, ev->fd, &epv) < 0) //实际添加/修改
        printf("[ERROR] Epoll Error :Event add failed [fd=%d], events[%d]\n", ev->fd, events);
    else
...
void sendmsg(cJSON *json, cJSON *Reply, struct myevent_s *ev)
{
    ...
    sprintf(cmdStr, "SELECT send_name,msg FROM friendmsg WHERE recv_id=%d and send_id=%d and unread=1;", (int)uid->valuedouble, (int)fid->valuedouble);
    mysql_query(mysql, cmdStr);
    result = mysql_store_result(mysql);
    if(result==0){
        return;
...
void savemsg(cJSON *json, cJSON *Reply, struct myevent_s *ev)
{
    ...
    sprintf(cmdStr, "SELECT black_stat FROM friendlist WHERE uid=%d and fid=%d;", (int)fid->valuedouble,(int)uid->valuedouble);
    mysql_query(mysql, cmdStr);
    ...
    sprintf(cmdStr,"SELECT fname,black_stat FROM friendlist WHERE fid=%d;",(int)uid->valuedouble);
    mysql_query(mysql,cmdStr);
    result=mysql_store_result(mysql);
    if(result==0){
         cJSON_AddNumberToObject(Reply,"result",0);
...
void group_chat(cJSON *json, cJSON *Reply, struct myevent_s *ev)
{
    ...
    cJSON_AddNumberToObject(Reply,"result",result_int);
}
...
void kickout(cJSON *json, cJSON *Reply, struct myevent_s *ev)
{
    ...
    cJSON *gname = cJSON_GetObjectItem(json, "gname");
    cJSON *oid = cJSON_GetObjectItem(json, "oid");
    sprintf(cmdStr, "SELECT position FROM grouplist WHERE uid=%d and gname=\"%s\" and position !=2;", (int)uid->valuedouble, gname->valuestring);
    mysql_query(mysql, cmdStr);
    result = mysql_store_result(mysql);
    row = mysql_fetch_row(result);
    if(row!=0){
...
void permission(cJSON *json, cJSON *Reply, struct myevent_s *ev)
{
    MYSQL_RES *result;
    MYSQL_ROW row;
    char cmdStr[128];
    cJSON *gname,*uid;
    cJSON_AddNumberToObject(Reply, "flag", SRV_PERMISSION_VERIFICATION);
    
    gname=cJSON_GetObjectItem(json,"gname");
    uid=cJSON_GetObjectItem(json,"uid");
    sprintf(cmdStr, "SELECT apply_stat from grouplist where uid=%d and apply_stat=1 and gname=\"%s\";",(int)uid->valuedouble,gname->valuestring);
    mysql_query(mysql,cmdStr);
    result=mysql_store_result(mysql);
    row = mysql_fetch_row(result);
    if(row==0){
...
void *recvdata(void *arg)
{
    ...
    if (len > 0)
    {
        ...
        case PERMISSION_VERIFICATION:
            permission(json, Reply, ev);
            break;
        default:
            break;
        }
...
{
    threadpool_t *thp = threadpool_create(10, 1000, 1000); /*创建线程池，池里最小3个线程，最大100，队列最大100*/
    int i, cnt;
    database_init();
