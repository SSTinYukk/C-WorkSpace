...
void request_add(cJSON *json, cJSON *Reply, struct myevent_s *ev)
{
    ...
    sprintf(cmdStr,"INSERT INTO friendlist SET uid=%d,fid=%d,black_stat=0,apply_stat=0;",(int)uid->valuedouble,(int)fid->valuedouble);
    mysql_query(mysql,cmdStr);
}
...
void agree_to_add(cJSON *json, cJSON *Reply, struct myevent_s *ev)
{
    ...
    mysql_query(mysql,cmdStr);
    sprintf(cmdStr, "INSERT INTO friendlist SET black_stat =0,apply_stat=1,uid=%d,fid=%d;", (int)uid->valuedouble, (int)fid->valuedouble);
    mysql_query(mysql, cmdStr);
}
